...
export default async function RegistrosPage() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: '0.8rem', color: 'var(--gris)', marginBottom: '2rem' }}>{total} inscripciones recibidas</p>
            ...
            {Object.entries(bloquesCount)
              ...
              .map(([bloque, count]) => (
                <div key={bloque} style={{ background: 'var(--crema2)', padding: '0.5rem 1rem', display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  <span style={{ fontSize: '1.1rem', fontFamily: 'Cormorant Garamond, serif', fontWeight: 600, color: 'var(--negro)' }}>{count}</span>
                  <span style={{ fontSize: '0.72rem', color: 'var(--gris)' }}>{bloque}</span>
                </div>
              ))}
          </div>
        </div>
                ...
                <tr key={r.id} style={{ borderBottom: '1px solid var(--crema3)', background: i % 2 === 0 ? 'var(--blanco)' : 'var(--crema)' }}>
                  ...
                  <td style={{ padding: '0.75rem 1rem', maxWidth: 280 }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                      {(r.bloques ?? []).map(b => (
                        <span key={b} style={{ fontSize: '0.58rem', letterSpacing: '0.1em', background: 'var(--amarillo)', color: 'var(--negro)', padding: '0.15rem 0.5rem', whiteSpace: 'nowrap' }}>
                          {b}
                        </span>
                      ))}
                    </div>
