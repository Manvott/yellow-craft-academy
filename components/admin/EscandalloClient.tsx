...
import type { ProductoEscandallo } from '@/app/[locale]/admin/(panel)/escandallo/page'
...
export default function EscandalloClient({ productos }: { productos: ProductoEscandallo[] }) {
  ...
  function abrirProducto(p: ProductoEscandallo) {
    ...
    const existentes = (p.combinaciones ?? [])
      ...
      .map(c => ({ id: c.id, nombre: c.nombre, peso: c.peso?.toString() ?? '', unidad: c.unidad, orden: c.orden }))
  ...
  const conEscandallo = productos.filter(p => (p.combinaciones?.filter(c => c.nombre).length ?? 0) > 0).length
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '2rem', marginBottom: '1.5rem' }}>
        ...
        <span style={{ fontSize: '0.8rem', fontFamily: 'DM Sans, sans-serif' }}>
          <strong style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.8rem', fontWeight: 300, color: '#16a34a' }}>{conEscandallo}</strong>
          <span style={{ color: 'var(--gris)', marginLeft: '0.4rem' }}>con escandallo</span>
        </span>
        <span style={{ fontSize: '0.8rem', fontFamily: 'DM Sans, sans-serif' }}>
          <strong style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.8rem', fontWeight: 300, color: 'var(--gris-l)' }}>{productos.length - conEscandallo}</strong>
          <span style={{ color: 'var(--gris)', marginLeft: '0.4rem' }}>sin escandallo</span>
        </span>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        ...
        />
        {([['todos','Todos'], ['con','Con escandallo'], ['sin','Sin escandallo']] as const).map(([key, label]) => (
              ...
              <div onClick={() => abrirProducto(p)}
                ...
                <div style={{ textAlign: 'right' }}>
                  {combsValidas.length > 0 ? (
                    ...
                    <span style={{ fontSize: '0.62rem', color: isActivo ? 'rgba(247,243,238,0.35)' : 'var(--gris-l)', fontStyle: 'italic' }}>Sin escandallo</span>
                  )}
                </div>
